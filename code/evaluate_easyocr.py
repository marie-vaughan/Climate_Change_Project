...
def extract_material(text):
    """
    Extract material composition by finding all percentage+word combinations
    This captures ANY fabric type without needing a predefined list
    """
    
    # Find all instances of: number% followed by word(s)
    # This pattern matches: "60% Cotton", "7% Spandex", "100% Merino Wool", etc.
    ...
    if matches:
        ...
        material = ' '.join(matches)
        
        # Clean up
        material = re.sub(r'/[A-Za-z]+', '', material)  # Remove /translations
...
def evaluate_ocr_accuracy(image_folder='tag_images', output_csv='ocr_evaluation.csv'):
    """
    Evaluate OCR accuracy on all images against ground truth
    """
    ...
    image_files = list(image_folder.glob('*.JPG')) + list(image_folder.glob('*.jpg'))
    
    # Filter to only images with ground truth
    image_files = [f for f in image_files if (image_folder / f"{f.stem}.txt").exists()]
    ...
    print(f"{'='*60}")
    print(f"Found {len(image_files)} images with ground truth")
    print(f"{'='*60}\n")
    ...
    for img_path in tqdm(image_files, desc="Processing images"):
        ...
        # Load ground truth
        txt_path = image_folder / f"{img_path.stem}.txt"
    ...
    for _, row in worst.iterrows():
        print(f"\n{row['image']} (error score: {row['error_score']:.2f}):")
