...
def run_paddle_ocr_single(img, ocr_engine):
...
def run_paddle_ocr_ensemble(img, ocr_engine, ground_truth):
    """Run PaddleOCR on multiple preprocessing variants"""
    variants = preprocess_variants(img)
    
    results = []
    
    for preprocessed_img, method_name in variants:
        text, confidence = run_paddle_ocr_single(preprocessed_img, ocr_engine)
    ...
    return {
        ...
        'method': f"paddle_ocr_{best_result['method']}",
...
def evaluate_ocr_accuracy(tag_images_folder='tag_images', 
                         ...
                         output_csv='ocr_evaluation_paddle.csv',
                         use_preprocessing=True):
    """Evaluate PaddleOCR accuracy with fuzzy-score-based preprocessing selection"""
    ...
    for img_path in tqdm(images_with_gt, desc="Processing images"):
        ...
        try:
            if use_preprocessing:
                ocr_result = run_paddle_ocr_ensemble(img, ocr, ground_truth)
            else:
                text, conf = run_paddle_ocr_single(img, ocr)
                ocr_result = {
                    'text': text or '',
                    'confidence': conf,
                    'method': 'paddle_ocr_original',
    ...
    print(f"\n{'='*60}")
    print("EVALUATION RESULTS (PADDLEOCR)")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    df = evaluate_ocr_accuracy(
        ...
        cropped_folder='cropped_tags',
        output_csv='ocr_evaluation_paddle_emissions.csv',
        use_preprocessing=True
    )
