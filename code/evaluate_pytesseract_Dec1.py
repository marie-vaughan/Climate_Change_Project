...
def run_pytesseract_single(img):
    """Run Pytesseract on a single preprocessed image"""
    try:
...
def run_pytesseract_ensemble(img, ground_truth):
    """Run Pytesseract on multiple preprocessing variants"""
    variants = preprocess_variants(img)
    
    results = []
    
    for preprocessed_img, method_name in variants:
        text, confidence = run_pytesseract_single(preprocessed_img)
    ...
    return {
        ...
        'method': f"pytesseract_{best_result['method']}",
...
def evaluate_ocr_accuracy(tag_images_folder='tag_images', 
                         ...
                         output_csv='ocr_evaluation_pytesseract_dec1.csv',
                         use_preprocessing=True):
    """Evaluate Pytesseract accuracy with fuzzy-score-based preprocessing selection"""
    
    print("\n" + "="*60)
    print("PYTESSERACT EVALUATION")
    ...
    for img_path in tqdm(images_with_gt, desc="Processing images"):
        ...
        try:
            if use_preprocessing:
                ocr_result = run_pytesseract_ensemble(img, ground_truth)
            else:
                text, conf = run_pytesseract_single(img)
                ocr_result = {
                    'text': text or '',
                    'confidence': conf,
                    'method': 'pytesseract_original',
...
if __name__ == "__main__":
    ...
    df = evaluate_ocr_accuracy(
        ...
        cropped_folder='cropped_tags',
        output_csv='ocr_evaluation_pytesseract_dec1_emissions.csv',
        use_preprocessing=True
    )
